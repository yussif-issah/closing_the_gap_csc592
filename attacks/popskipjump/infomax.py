...
def get_bernoulli_probs(xx, unperturbed, perturbed, model_interface, label, dist_metric='l2', targeted=False):
    ...
    probs = model_interface.get_probs(batch)
...
def bin_search(
    ...
    ttssee = torch.stack(torch.meshgrid(tt, ss, ee, indexing='ij'))  # 2 x Nt x Ns  (numpy indexing='ij')
    ll = zz[:, None] - tt[None, :]  # distance matrix: Nz x Nt
    llse, lsse, lsee = torch.meshgrid(ll.flatten(), ss, ee, indexing='ij')
    ...
    if plot:
        ttss = torch.stack(torch.meshgrid(tt, ss, indexing='ij'))  # 2 x Nt x Ns  (numpy indexing='ij')
    ...
    Y, T, X, S, E = torch.meshgrid(yy, tt, xx, ss, ee, indexing='ij')
    ...
    for k in range(krepeat):
        ...
        if human_interface is None:
            if model_interface is None:
                ...
                pj = get_bernoulli_probs(xj[None], unperturbed, perturbed, model_interface, label, dist_metric,
                                         targeted)
