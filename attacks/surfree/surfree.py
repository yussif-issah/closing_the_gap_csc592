...
import attacks.surfree.utils.dct as torch_dct
from attacks.surfree.utils.attack import get_init_with_noise
from attacks.surfree.utils.utils import atleast_kdim

# Taken from https://github.com/t-maho/SurFree
...
class SurFree():
    ...
    def __call__(self, model, X, labels, starting_points=None, **kwargs):
        ...
        # Get Starting Point
        self.best_advs, indices = get_init_with_noise(model, X, labels) if starting_points is None else starting_points
        ...
        print(f"Already advs: {self._images_finished.cpu().tolist()}")
        ...
        for step_i in range(self._steps):
            ...
            try:
                ...
                print("Error occured, breaking loop")
                break
            ...
            print(f"Best Advs distance:{distance(X, self.best_advs).cpu().numpy()}")
        ...
        return self.best_advs, X, labels
